...
var _reqHeadersWhiteList = []

exports = module.exports = function (headers) { 
  _reqHeadersWhiteList = _reqHeadersWhiteList.concat(headers)
...
function HttpProxyMiddleware (context, opts) {
  ...
  function middleware (req, res, next) {
    if (!shouldProxy(config.context, req)) {
      ...
      var proxyReq = http.request(activeProxyOptions, function (proxyRes) {
        ...
        proxyRes.on('end', function () {
          try {
            ...
            writeHeaders(proxyRes, res)
            // res.writeHead(200, {'Content-Type': 'application/json;charset=utf-8'})
            res.end(content)
          } catch (err) {
            logger.error(`[tc] ${Date.now()} problem with response url ${req.url} : ${err.message}`)
      ...
      proxyReq.on('error', function (e) {
        logger.error(`[tc] ${Date.now()} problem with request ${req.url} : ${e.message}`)
        res.writeHead(500)
        res.end()
      })
      if(postData) {
  ...
  function prepareProxyRequest (req) {
    ...
    if (proxyOptions.logLevel === 'debug') {
      ...
      logger.debug('[TC] %s %s %s %s', req.method, originalPath, arrow, newProxyOptions.target)
    }
  ...
  function _applyRequestOptions (req, options) {
    ...
    _reqHeadersWhiteList.map(function(h) {
      if(req.headers[h]) reqOptions.headers[h] = req.headers[h]
    })
  ...
  function writeHeaders(proxyRes, res) {
    var rawHeaderKeyMap,
